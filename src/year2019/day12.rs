...
mod moons {
    ...
    impl Moons {
        pub fn new(input: &[String]) -> Moons {
            let mut moons = [[(0, 0); 4]; 3];
            for (moon, line) in input.iter().enumerate() {
                let mut parts = line[1..line.len() - 1].split(", ");
                for axis in 0..3 {
                    moons[axis][moon] = (parts.next().unwrap()[2..].parse().unwrap(), 0);
                }
            }
            Moons { moons }
        }

        pub fn step_all(&mut self, iterations: usize) {
        ...
        pub fn step_until_repeat(&mut self, axis: usize) -> usize {
